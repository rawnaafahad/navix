"""CleanRL-style PPO training loop for the DISCOVERED-option NetHack
policy -- the third arm of the 3-way comparison (primitive vs hand-coded
options vs discovered options).

v2 CHANGE: budgeted by TOTAL PRIMITIVE ENVIRONMENT STEPS, not option-
selection steps. A 16-step discovered/hand-coded option and a single
primitive action are NOT equal units of environmental experience, so
comparing tracks at "the same number of option-steps" silently gives
option-based tracks a much larger real interaction budget than the
primitive-action baseline at the same nominal step count. This matters
...
step budget is not known in advance (it depends on how many primitive
steps each option call actually consumes, which varies), the LR
annealing schedule's denominator is an explicit CLI parameter
(--anneal-total-updates) rather than an auto-computed constant -- set it
from a short calibration run before a real training run.
"""
...
from discovered_option_env import (
    DiscoveredOptionEnv,
    DiscoveredOptionEnvConfig,
    NUM_GLYPHS,
    CROP_SIZE,
    NUM_FEATURES,
)
...
class PPOConfig:
    ...
    max_option_steps: int = None  # None = default (FUTURE_HORIZON cap, unchanged behaviour)
    save_path: str = "discovered_ppo_checkpoint.pkl"
...
def train(config: PPOConfig):
    ...
    print(
        f"Training seed pool: [{config.train_seeds_start}, {config.train_seeds_end}) "
    ...
    env_config = DiscoveredOptionEnvConfig(
        ...
        use_random_grounding=config.use_random_grounding,
        random_grounding_seed=config.random_grounding_seed,
        max_option_steps=config.max_option_steps,
    )
    env = DiscoveredOptionEnv(env_config)
    print(f"Discovered options loaded: {env.num_options}")

    network = OptionActorCritic(num_options=env.num_options, num_glyphs=NUM_GLYPHS)
...
def main():
    ...
    parser.add_argument("--policy-dir", type=str, default="bc_policies_cropped")
    parser.add_argument("--use-random-grounding", action="store_true",
        help="Ablation: use randomly-initialized BC policy weights instead "
             "of trained ones, keeping the same architecture/action-vocab.")
    parser.add_argument("--random-grounding-seed", type=int, default=0)
    parser.add_argument("--max-option-steps", type=int, default=None,
        help="Diagnostic-only: caps primitive steps consumed per option call "
             "(independent of FUTURE_HORIZON/stuck-loop). Default None preserves "
             "existing behaviour unchanged. Used to test whether trained options "
             "returning control less often than random-grounding explains the "
             "held-out success-rate gap.")
    parser.add_argument("--ent-coef", type=float, default=0.01)
    parser.add_argument("--save-path", type=str, default="discovered_ppo_checkpoint.pkl")
    ...
    config = PPOConfig(
        ...
        use_random_grounding=args.use_random_grounding,
        random_grounding_seed=args.random_grounding_seed,
